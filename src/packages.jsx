...
function Packages() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await emailjs.send(
        SERVICE_ID,
        TEMPLATE_ID,
        { ...formData, destination: selectedPackage?.title || "" },
        PUBLIC_KEY
      );
  ...
  return (
    <div className="min-h-screen font-sans bg-[#0f1b2d] text-white">
      ...
      <section className="relative w-full overflow-hidden min-h-[30vh] flex items-center">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center w-full py-16">
        ...
        <div className="space-y-12">
          {packages.map((pkg, index) => {
            ...
            return (
              <article
                key={pkg.id}
                className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} gap-8 lg:gap-12 rounded-3xl overflow-hidden border border-white/10 shadow-lg hover:shadow-2xl transition-all bg-white/5`}
              >
                {/* Image */}
                <div className="lg:w-5/12 relative overflow-hidden h-80 lg:h-auto">
                  <img
                    src={pkg.image}
                    alt={pkg.title}
                  ...
                  {/* Duration */}
                  <div className="absolute bottom-6 left-6 bg-black/60 backdrop-blur-sm text-white text-sm font-medium px-4 py-2 rounded-2xl flex items-center gap-2 shadow">
                    <span>⏱️</span>
                    {pkg.duration}
                  </div>
                </div>
                ...
                <div className="lg:w-7/12 p-6 sm:p-8 lg:p-10 flex flex-col">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm font-medium text-white/80">{pkg.location}</span>
                    </div>
                    ...
                    {/* Highlights */}
                    <div className="mb-8">
                      <p className="text-xs font-semibold text-white/50 mb-4 tracking-widest uppercase">
                        What's Included
                      </p>
                  ...
                  <div className="mt-auto pt-6 flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                      ...
                      className="border border-white/30 hover:border-orange-500 hover:text-orange-500 text-white font-semibold py-3.5 px-8 rounded-2xl transition-all active:scale-95"
                    >
                      View Details
                    </button>
                  </div>
      ...
      {isEnquiryOpen && (
        <div
          ...
          onClick={(e) => e.target === e.currentTarget && setIsEnquiryOpen(false)}
        >
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <div className="px-6 sm:px-8 py-6 border-b bg-orange-50 flex justify-between items-center">
              <div>
                ...
                <h3 className="font-bold text-lg sm:text-xl text-gray-900 mt-1">
            ...
            <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-2xl focus:outline-none focus:border-orange-500 transition-colors"
                />
              </div>
  ...
  <textarea
    ...
    className="w-full px-4 py-3 border border-gray-200 rounded-2xl text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-orange-500 transition-colors resize-y"
  />
</div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white font-bold py-4 rounded-2xl transition-all mt-2 flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
